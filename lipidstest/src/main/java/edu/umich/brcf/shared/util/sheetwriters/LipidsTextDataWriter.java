...
public class LipidsTextDataWriter implements Serializable, IWriteableTextData
	{
	protected Ms2DataSetHandler dataHandler;
	
	public LipidsTextDataWriter(Ms2DataSetHandler handler)
	{
	this();
	...
	public void generateTextReport(OutputStream output)
		{
		createTextFile("Worklist", dataHandler);
	...
	public String createTextFile(String title, Ms2DataSetHandler handler)
		{
		...
		int rowCt = 0;
		//Sheet sheet = createEmptySheet(dataHandler.getDataSet().getIonMode(),  workBook, 1, dataHandler, colTitles);
		
		//PoiUtils.createBlankRow(rowCt,  sheet);
		
		//for (int i = 0; i < colTitles.size(); i++)
		//PoiUtils.createRowEntry(rowCt, i, sheet, colTitles.get(i), grabStyleBlue(workBook));
		...
		for (Ms2PeakSet item : peakSets)
			...
			//Cell cell = PoiUtils.createRowEntry(rowCt, i, sheet, tokens[i], style);
			//}
			rowCt++;
			}
		...
		public Sheet createEmptySheet(String title, Workbook workBook, int page, Ms2DataSetHandler dataHandler, List<String> headers)
		    ...
		    for (int i = 0; i < headers.size(); i++) 
		    	...
		    	if (i == 1)
		    		...
		    		sheet.setColumnWidth(i, i > 7 ?  20 * 256 : 20 * 256);
		    	}
...
return new StringResourceStream(builder.toString(), mimeType);
		...
		public String createWorklistSheet(String title, Ms2DataSetHandler handler)
			{
