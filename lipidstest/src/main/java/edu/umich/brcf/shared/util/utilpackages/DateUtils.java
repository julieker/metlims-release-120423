...
public class DateUtils 
	...
	public static String dateAsFullString(Date date)
		{
		if (date == null) 
			return "";
		
		String dateStr = dateAsString(date, "MM/dd/yy");
		String [] tokens = StringUtils.splitAndTrim(dateAsString(date, "MM/dd/yy"), "/");
		
		if (tokens.length != 3) 
			return "";
		
		if (tokens[0].length() > 2 || tokens[0].length() <= 0)
			return "";
		...
		String monthStr = tokens[0].length() == 2 ? tokens[0] :  "0" + tokens[0];
		String dayStr = tokens[1].length() == 2 ? tokens[1] : "0" + tokens[1];
		
		return monthStr + "/" + dayStr + "/" + tokens[2];
		}
