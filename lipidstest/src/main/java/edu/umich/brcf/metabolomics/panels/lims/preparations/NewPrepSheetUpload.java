...
import edu.umich.brcf.metabolomics.panels.lims.prep.FractionPrepPanel;
...
public class NewPrepSheetUpload extends WebPage{
	...
	public NewPrepSheetUpload(Page backPage, String title, NewNewPreparationsSearchPanel pp, String plateFrmt){
		add(new FeedbackPanel("feedback"));
		add(new PSUForm("psuForm", title, pp, plateFrmt));
	}
	
	public NewPrepSheetUpload(Page backPage, String title, FractionPrepPanel fp){
		add(new FeedbackPanel("feedback"));
	...
	public final class PSUForm extends Form 
		{
		public PSUForm(final String id, final String title, final NewNewPreparationsSearchPanel pp, final String plateFrmt){
			super(id);
	        ...
	        add(new Button("upload"){
	       	 	...
	       	 	public void onSubmit()
	            {
	       			...
	       			NewPrepSheetUpload.this.info(messages[0]);
	       			pp.setPreparation(messages[1]);
	        		}
	        		else{
	        			NewPrepSheetUpload.this.error(retStr);
