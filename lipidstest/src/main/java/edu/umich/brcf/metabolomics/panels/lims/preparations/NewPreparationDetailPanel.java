...
import edu.umich.brcf.metabolomics.panels.lims.prep.GCPrepDetail;
import edu.umich.brcf.metabolomics.panels.lims.prep.GeneralPrepDetail;
import edu.umich.brcf.metabolomics.panels.lims.prep.HomogenizationDetail;
import edu.umich.brcf.metabolomics.panels.lims.prep.LCPrepDetail;
...
public class NewPreparationDetailPanel  extends Panel{
...
//	InjectionsService injectionsService;
	...
	public NewPreparationDetailPanel(String id, String preparation) {
		super(id);
		...
		add(new ListView("sampleList", new PropertyModel(this, "sampleList"))
			...
			public void populateItem(final ListItem listItem)
				...
				boolean wellNull = (sample.getWell() == null);
				
				listItem.add(new Label("index", (wellNull ? new Model("") : new Model(sample.getWell().getIndex()))));
				...
				listItem.add(new Label("well", new Model(wellNull ? "" : sample.getWell().getLocation())));
...
//				listItem.add(new Button("report"));
				...
				//WebPage runPage=new SampleRunPage(plate);
				//listItem.add(buildLinkToModal("run", modal1, runPage));
				listItem.add(buildWorklistBuilderLink("worklist"));
				listItem.add(OddEvenAttributeModifier.create(listItem));
			}
		});
	...
	public void setPreparation(String preparation)
		...
		if((preparation!=null)&&(preparation.length()>0))
			{
			setSampleList(samplePrepService.loadPreppedSamplesInNewOrder(preparation));
